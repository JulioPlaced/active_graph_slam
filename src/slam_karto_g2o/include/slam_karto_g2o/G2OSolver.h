...
class G2OSolver : public karto::ScanSolver
{
  ...
  public:
    ...
    void getGraphWithFIM(std::vector<Eigen::Vector2d> &nodes, std::vector<std::pair<Eigen::Vector2d, Eigen::Vector2d> > &edges, std::vector< Eigen::VectorXd > &infoMatrices);
    ...
    void setGraphDir(std::string dir)
    {
        graphDirectory_ = dir;
    }
